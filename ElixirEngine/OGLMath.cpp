...
namespace ogldev
{
	...
	void Vector3f::Rotate(float Angle, const Vector3f& Axe)
	{
		...
		OGL_Quaternion RotationQ(Rx, Ry, Rz, Rw);

		OGL_Quaternion ConjugateQ = RotationQ.Conjugate();
		//  ConjugateQ.Normalize();
		OGL_Quaternion W = RotationQ * (*this) * ConjugateQ;
	...
	void Matrix4f::InitRotateTransform(const OGL_Quaternion& quat)
	{
	...
	OGL_Quaternion::OGL_Quaternion(float _x, float _y, float _z, float _w)
	{
		x = _x;
		y = _y;
		z = _z;
	...
	OGL_Quaternion OGL_Quaternion::Conjugate()
	{
		OGL_Quaternion ret(-x, -y, -z, w);
		return ret;
	}
	OGL_Quaternion operator*(const OGL_Quaternion& l, const OGL_Quaternion& r)
	{
	...
	Vector3f OGL_Quaternion::ToDegrees()
	{
		float f[3];
